...
from seamseg.modules.misc import GlobalAvgPool2d
from seamseg.modules.residual import ResidualBlock
...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_group=32):
        ...
        # 4 layers depth
        self.layer1 = self._make_layer(block, 64, layers[0], num_group)             # x3 layer
        self.layer2 = self._make_layer(block, 128, layers[1], num_group, stride=2)  # x4
        self.layer3 = self._make_layer(block, 256, layers[2], num_group, stride=2)  # x6
        self.layer4 = self._make_layer(block, 512, layers[3], num_group, stride=2)  # x3
    ...
    def _make_layer(self, block, planes, iter_blocks, num_group, stride=1):
        downsample = None
        ...
        for i in range(1, iter_blocks):   #{ 3* 4 * 6 *3}
